...
#pragma omp parallel for private(u,x,du,ddu,Mxi)
  for ( i = 1; i < N-1; i++) {
    ...
    gsl_vector_set(fu, i,
		   (ddu+u-(k-1.)/2.*sin(2.*u/sin(x))/sin(x)));
    gsl_vector_set(ftmp, i,
		   1./epsilon*Mxi);
    gsl_matrix_set(C, i, i, -du/* +u/tan(x) */);
  }
  ...
  for ( i = 1; i < N-1; i++)
    {
      du=D1( u, x, i, N );
      ddu=D2( u, x, i, N );
      M[i]=fabs(( du - u[i]/tan(x[i]) )/sin(x[i]))
	+ sqrt(fabs((ddu-2.*du/tan(x[i])+u[i]*(1.+2./tan(x[i])/tan(x[i])))/sin(x[i])));
