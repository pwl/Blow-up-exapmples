...
echo "set output \"graphics/Passing_through_the_blow_up_log.ps\"" >> plotter.gp
...
for snap in $snapshot_files; do
	...
	echo 'set xtics (-15,-5,0,5,15)' >> plotter.gp
	echo "set xlabel \"log(tan({/Symbol f}/2))\"" >> plotter.gp
	echo "set ylabel \"u(t,{/Symbol f})\" rotate offset screen .1*$sizex, 0." >> plotter.gp
    fi

    echo "set title \"t=$t\" offset screen .05*$sizex, screen -1.1*$sizey font \"Times-Roman,10\"" >> plotter.gp
    echo -ne "plot [-15:15] [0:3*pi] pi lt 2, 2*pi lt 2, \"$snap\" u (log(tan(\$1/2))):(\$2/sin(\$1)) w l lw 2 lt 1\n" >> plotter.gp
...
ps2pdf graphics/Passing_through_the_blow_up_log.ps graphics/Passing_through_the_blow_up_log.pdf
evince graphics/Passing_through_the_blow_up_log.ps
